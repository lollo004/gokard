...
@export var client_id = "client1" + str(randi_range(1,10000)) #debug only !!
...
func handle_new_message(message): # Check message type and choose what to do
	...
	if get_tree().get_first_node_in_group("output") and message["type"] != "user_event": ## DEBUG ONLY
		get_tree().get_first_node_in_group("output").append_text(str(message) + "\n\n")
		get_tree().get_first_node_in_group("output").scroll_to_line(get_tree().get_first_node_in_group("output").get_line_count())
		#print("Seed: " + str(Data.RANDOM.get_state()))
...
#				return
		if message["action"] == "142": # Opponent tell you where and who he played
			...
			Data.RANDOM.set_seed(hash(message["rnd_seed"])) # randomize seed according to other client
...
func send_effect_114(rnd): # Function called when you play card with id 114
	var join_message = {
		"type": "game_effect",
		"action": "114",
		"id": client_id,
		"card_id": GameController.player_hand[rnd].id #card
	}
...
func send_effect_142(pos, card_id, rnd_seed): # Function called when you play card with id 142
	var join_message = {
		"type": "game_effect",
		"action": "142",
		"id": client_id,
