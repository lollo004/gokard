...
func Effect(who, _death, _n): # After he attacks he deal 1 damage to an enemy
	if who.Team == "player" and card == who and len(GameController.enemy_field_cards) > 0:
		var rnd = randi_range(0, len(GameController.enemy_field_cards) - 1)
		...
		GameController.enemy_field_cards[rnd].BoostByPos(GameController.enemy_field_cards[rnd].Position, "health", -1, "enemy") # Boost attack
